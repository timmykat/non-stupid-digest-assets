  ...
  class Manifest
    def compile_with_non_digest *args
      ...
      NonStupidDigestAssets.files(files).each do |(digest_path, info)|
        ...
        if File.exists? full_digest_path and !(full_digest_path == full_non_digest_path)
        ...
        if File.exists? full_digest_gz_path and !(full_digest_gz_path == full_non_digest_gz_path)
